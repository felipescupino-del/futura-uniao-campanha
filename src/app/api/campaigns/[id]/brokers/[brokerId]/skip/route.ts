import { NextRequest, NextResponse } from 'next/server';
import { prisma } from '@/lib/prisma';

export async function POST(
  _req: NextRequest,
  { params }: { params: Promise<{ id: string; brokerId: string }> },
) {
  const { id, brokerId } = await params;

  const cb = await prisma.campaignBroker.findFirst({
    where: {
      campaignId: Number(id),
      brokerId: Number(brokerId),
    },
    include: {
      campaign: {
        include: { steps: { orderBy: { stepNumber: 'asc' } } },
      },
    },
  });

  if (!cb) return NextResponse.json({ error: 'Not found' }, { status: 404 });

  const nextStep = cb.currentStep + 1;

  if (nextStep > cb.campaign.totalSteps) {
    await prisma.campaignBroker.update({
      where: { id: cb.id },
      data: { status: 'unresponsive', nextMessageAt: null },
    });
  } else {
    const step = cb.campaign.steps.find((s) => s.stepNumber === nextStep);
    const delayMs = (step?.delayDays || 1) * 24 * 60 * 60 * 1000;
    await prisma.campaignBroker.update({
      where: { id: cb.id },
      data: {
        currentStep: nextStep,
        nextMessageAt: new Date(Date.now() + delayMs),
      },
    });
  }

  return NextResponse.json({ success: true });
}
